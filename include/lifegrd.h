 ...
 *  The heartbeat mechanism for a device is established through cyclically
 * transmitting a message by a heartbeat producer. One or more devices in the
 * network are aware of this heartbeat message. If the heartbeat cycle fails for
 * the heartbeat producer the local application on the heartbeat consumer will
 * be informed about that event.
 *  @ingroup comobj
 */

/** @defgroup nodeguardo Node-guarding Object
 *  The node-guarding mechanism for a device is established through cyclically
 * polling all slaves by the NMT master. If one polled slave does not respond
 * during a specified time (LifeTime), the local application will be informed
 * about that event.<br> It is also possible for the slaves to monitor the
 * node-guarding requests coming from the master to determine, if the master
 * operates in a right way
 *  @ingroup comobj
 *
 * @todo The implementation is very basic. The toggle bit of the nodes
 * confirmation is not checked at the moment
 */
...
/**
 * @brief This function is responsible to process a canopen-message which seams
 * to be an NMT Error Control Messages. If a BootUp message is detected, it will
 * return the nodeId of the Slave who booted up
 * @param *d Pointer on a CAN object data structure
