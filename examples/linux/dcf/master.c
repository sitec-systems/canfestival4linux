...
void slave_state_change_callback(CO_Data* d, UNS8 nodeId,
                                 e_nodeState newNodeState) {
...
int main(int argc, char** argv) {
    ...
    // Set the master node id, we do not call setNodeId() because we do not need
    // all the predefined connexion set
    *masterdic_Data.bDeviceNodeId = 100;
    ...
    if (LoadCanDriver("/usr/lib/libcanfestival_can_socket.so") == NULL) {
        printf("Unable to load driver library\n");
        printf(
            "please put file libcanfestival_can_socket.so in the current "
            "directory\n");
        exit(1);
    }
    ...
    /* Put the master in Pre_operational mode, this will broadcast a
     * comunication reset */
